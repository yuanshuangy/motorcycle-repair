...
public class DataInitializer implements CommandLineRunner {
    ...
    private void migrateDatabase() {
        try (Connection conn = dataSource.getConnection()) {
            addColumnIfNotExists(conn, "appointment", "wait_auto_assign", "INT DEFAULT 0");
            addColumnIfNotExists(conn, "appointment", "estimated_wait_minutes", "INT DEFAULT NULL");
            addColumnIfNotExists(conn, "appointment", "wait_discount_rate", "DOUBLE DEFAULT NULL");
            addColumnIfNotExists(conn, "appointment", "cancel_reason", "VARCHAR(500) DEFAULT NULL");
            addColumnIfNotExists(conn, "appointment", "estimated_complete_time", "DATETIME DEFAULT NULL");
            log.info("数据库迁移完成");
        } catch (Exception e) {
            log.warn("数据库迁移异常: {}", e.getMessage());
        }
    }
    ...
    private void initServiceTemplate() {
        ...
        String[][] svcs = {
            ...
            {"火花塞更换","60.00","20","火花塞拆卸、更换、间隙调试","点火","11"},
            {"空滤清洗/更换","40.00","20","空气滤芯清洁、滤芯更换","保养","12"},
            {"电瓶检测/充电","30.00","20","电瓶健康检测、亏电充电维护","电气","13"},
            {"大灯/转向灯维修","50.00","30","灯光线路检查、灯泡更换、灯壳调试","电气","14"},
            {"离合系统检修","120.00","40","离合线调整、离合片检查更换","传动","15"},
            {"换挡机构检修","90.00","40","换挡连杆调试、挡位卡顿维修","传动","16"},
            {"燃油系统清洗","130.00","50","油路疏通、喷油嘴/燃油滤芯维护","燃油","17"},
            {"脚踏/挂挡杆维修","70.00","30","脚踏修复、挂挡杆更换调试","外观","18"},
            {"后视镜/车把维修","80.00","30","后视镜更换、车把校正、握把更换","外观","19"},
            {"排气管检修","100.00","40","排气管漏气维修、吊耳更换","排气","20"},
            {"整车检查","80.00","40","全车安全检测、故障排查","保养","21"},
            {"洗车+链条养护","50.00","30","车身精洗、链条清洗上油","保养","22"},
        };
        for (String[] s : svcs) {
