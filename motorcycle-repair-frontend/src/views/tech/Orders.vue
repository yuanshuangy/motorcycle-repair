        ...
        <el-table-column label="完单/更新时间" width="170">
          <template #default="{row}">
            ...
            <div v-else-if="(row.status===1 || row.status===2) && row.estimatedCompleteTime" style="font-size:12px">
              <div :style="{color: getTechCountdownColor(row), fontWeight: 700}">⏱ {{ getTechCountdownText(row) }}</div>
...
const techCountdownTick = ref(Date.now())
let techCountdownTimer = null
const getTechCountdownText = (row) => {
  techCountdownTick.value
...
const getTechCountdownColor = (row) => {
  techCountdownTick.value
...
const startTechCountdown = () => {
  stopTechCountdown()
  techCountdownTimer = setInterval(() => { techCountdownTick.value = Date.now() }, 1000)
}
const stopTechCountdown = () => {
  if (techCountdownTimer) { clearInterval(techCountdownTimer); techCountdownTimer = null }
}
onMounted(() => { fetchData(); startTechCountdown() })
onBeforeUnmount(stopTechCountdown)
</script>
<style scoped>
