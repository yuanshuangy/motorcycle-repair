        ...
        <el-table-column label="完单/更新时间" width="170">
          <template #default="{row}">
            ...
            <div v-else-if="(row.status===1 || row.status===2) && row.estimatedCompleteTime" style="font-size:12px">
              <div :style="{color: getShopCountdownColor(row), fontWeight: 700}">⏱ {{ getShopCountdownText(row) }}</div>
...
const openAssign = async row => {
  ...
  try {
    const [techR, empR, restR] = await Promise.all([shopAPI.getTechnicians(shopId.value), employeeAPI.getByShop(shopId.value), shopAPI.getTomorrowRestTechs(shopId.value)])
    if(techR.code===200) {
...
const applyFilter = () => {}
const shopCountdownTick = ref(Date.now())
let shopCountdownTimer = null
const getShopCountdownText = (row) => {
  shopCountdownTick.value
...
const getShopCountdownColor = (row) => {
  shopCountdownTick.value
...
const startShopCountdown = () => {
  stopShopCountdown()
  shopCountdownTimer = setInterval(() => { shopCountdownTick.value = Date.now() }, 1000)
}
const stopShopCountdown = () => {
  if (shopCountdownTimer) { clearInterval(shopCountdownTimer); shopCountdownTimer = null }
}
onMounted(() => { fetchData(); startShopCountdown() })
onBeforeUnmount(stopShopCountdown)
</script>
<style scoped>
