<template>
  <div>
    ...
    <el-card>
      ...
      <el-pagination v-model:current-page="pageNum" v-model:page-size="pageSize" :total="total" :page-sizes="[10,20,50]" layout="total, sizes, prev, pager, next, jumper" @size-change="fetchData" @current-change="fetchData" style="margin-top:20px;justify-content:center" />
    </el-card>
...
const fetchData = async () => { try { const r=await shopAPI.getByUserId(userStore.userId); if(r.code===200&&r.data) { shopId.value=r.data.id; const ar=await appointmentAPI.getPage({pageNum:pageNum.value,pageSize:pageSize.value,shopId:shopId.value}); if(ar.code===200) { list.value=ar.data.records; total.value=ar.data.total } } } catch(e){} }
